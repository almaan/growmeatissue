...
class Cell:
    def __init__(self,
        ...
        if action_rates is not None:
            ...
            self.action_rates /= self.action_rates.sum(axis=0,
                                                    keepdims=True)
        else:
        ...
        assert np.all(self.action_rates >= 0),\
            "rates must be non-negative than zero"
...
def grow_tissue(n_initial_cells: int,
                genome: Genome,
    ...
    if init_coordinates is None:
        ...
        for i in range(n_initial_cells):
            xy = np.random.uniform(0.1 * domain_side_size,
                                   0.9 * domain_side_size,
                                   size = 2,
                                   )
            init_coordinates.append(xy)
    else:
        if isinstance(init_coordinates[0],int) or\
           isinstance(init_coordinates[0],float):
            init_coordinates = [init_coordinates]

            assert len(init_coordinates) == n_initial_cells,\
                "incorrect number of initial coordinate tuples"
            assert len(init_coordinates[0] == 2),\
                       "coordinate _tuple_ is required"

            for ic in init_coordinates:
                for ii in range(len(ic)):
                    assert ic[ii] > 0 & ic[ii] < domain_side_size,\
                        "Initial coordinate pair ({},{}) is out of bounds".format(ic[0],ic[1])
    ...
    for i in range(n_initial_cells):
        ...
        seed_cell = Cell(genome = copy.deepcopy(genome),
                         counter = counter,
                         mutation_rate = mutation_rate,
                         action_rates = action_rates,
                         tmat = tmat,
        ...
        init_event_genes = np.arange(start_event,
                                     start_event + init_event_size)
        if i % 2 == 0:
...
def add_background(population: List[Cell],
                   genome: Genome,
                   cell_map: np.ndarray,
    ...
    if fraction_drop > 0:
        drop = np.random.choice(pop_size,
                                replace=False,
                                size=int(pop_size * fraction_drop))
    ...
    for x,y in zip(row,col):
        normal_population.append(Cell(x=x,y=y,
