...
def run(
    ...
    population, lineage, cell_map = m.grow_tissue(
        n_initial_cells,
        genome=genome,
    ...
    cluster_idx, cluster_genome = f.add_cluster(
        population,
        n_clusters,
    )

    cluster_lineage = f.get_cluster_lineage(population, cluster_idx, lineage)

    # TODO: save clustering relations
    logger.info("Completed : Clustering")

    logger.info("Initialized : Add normal cells")
    population, dropped = m.add_background(
        population,
        genome,
        cell_map,
        fraction_drop,
    ...
    cluster_idx = np.append(
        cluster_idx,
        (max(cluster_idx) + 1) * np.ones(len(population) - len(cluster_idx)),
    )

    logger.info(
        "Completed : Add normal cells | Total Cells : {}".format(len(population))
    )
    ...
    lineage = np.array(
        [(c, p) for k, (c, p) in enumerate(lineage.items()) if k not in dropped]
    )
...
def main() -> None:
    ...
    if args.random_seed is not None:
        try:
            ...
            logger.warning("Failed to set random seed. Will continue")
    ...
    if (
        "rates_state_0" in ds["population"].keys()
        and "rates_state_1" in ds["population"].keys()
    ):
        ...
        mutation_rate = np.array((rates_0["mutation_rate"], rates_1["mutation_rate"]))
    ...
    dargs = dict(
        n_initial_cells=ds["population"]["n_initial_cells"],
    ...
    if args.visual_output:
        fig, ax = ut.visualize_tissue(
            sc_data["genome_profile"],
            sc_data["coordinates"],
        )
    ...
    sc_genome_profile = pd.DataFrame(
        sc_data["genome_profile"],
        columns=gene_names,
        index=cell_names,
    )

    sc_meta_data = pd.DataFrame(
        np.hstack((sc_data["coordinates"], sc_data["cluster_index"][:, np.newaxis])),
        columns=["x", "y", "cluster"],
        index=cell_names,
    )
    ...
    st_data = res["st_data"]
    spot_names = pd.Index(
        ["Spot_{}".format(x) for x in range(st_data["expression"].shape[0])]
    )

    if args.visual_output:
        fig, ax = ut.visualize_array(
            sc_data["genome_profile"],
            sc_data["coordinates"],
    ...
    st_genome_profile = pd.DataFrame(
        st_data["genome_profile"],
        columns=gene_names,
        index=spot_names,
    )

    st_meta_data = pd.DataFrame(
        st_data["coordinates"],
        columns=["x", "y"],
        index=spot_names,
    )
    ...
    if st_data["cluster_index"] is not None:
        cluster_names = pd.Index(
            ["Cluster_{}".format(x) for x in range(st_data["cluster_index"].shape[1])]
        )

        cluster_data = pd.DataFrame(
            st_data["cluster_index"],
    ...
    st_annotation.annotation = st_annotation.annotation.map(
        {1: "benign", 0: "aberrant"}
    )
    ...
    cluster_names = pd.Index(
        ["Cluster_{}".format(x) for x in range(res["cluster_lineage"].shape[0])]
    )
    ...
    for df, name in zip(
        [cluster_lineage, lineage, genome_data],
